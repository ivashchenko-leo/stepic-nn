...
class Network:
    ...
    def backprop(self, x, y):
        ...
        summatory = [] # list to store all the z vectors, layer by layer
        for b, w in zip(self.biases, self.weights):
        ...
        delta = (self.cost_derivative(activations[-1], y) * sigmoid_prime(summatory[-1]))# ошибка выходного слоя
        ...
        for l in range(2, self.num_layers):
            # ошибка на слое L-l
            delta = (np.dot(self.weights[-l + 1].T, delta) * sigmoid_prime(summatory[-l]))
...
x = np.array([[1], [2], [3]])
y = np.array([[0], [1]])
#вызов nn.backprop(x, y) должен вернуть кортеж из двух списков длины 1, содержащих производные по этим весам и смещениям соответственно:
